...
export const InvoiceEditPage = () => {
    ...
    const { states, set } = useStates({
        ...
        socid: initialSocId,
        refClient: "",
        datef: tsToDateStr(Math.floor(Date.now() / 1000)),
        dateLimReglement: "",
        notePublic: "",
        notePrivate: "",
        lineLabel: "",
    ...
    const {
        invoice, loading, saving, error,
        socid, refClient, datef, dateLimReglement, notePublic, notePrivate,
    ...
    const loadInvoice = async () => {
        ...
        try {
            ...
            set("datef", tsToDateStr(data?.datef));
            set("dateLimReglement", tsToDateStr(data?.dateLimReglement));
    ...
    const handleSave = async () => {
        if (!socid || socid <= 0) {
            await alert({
                ...
                message: "Indiquez un identifiant de tiers (socid) pour la facture.",
            });
            return;
        }
        set("saving", true);
        ...
        try {
            const payload = {
                ...
                fkSoc: Number(socid),
                refClient: refClient ?? "",
                datef: dateStrToTs(datef),
                dateLimReglement: dateStrToTs(dateLimReglement),
                notePublic: notePublic ?? "",
                notePrivate: notePrivate ?? "",
            };
            if (isNew) {
                const data = await dbInvoices.create(payload);
                if (data?.id) {
                ...
                const data = await dbInvoices.update(id, payload);
                set("invoice", data);
            }
        } catch (err) {
            console.error("dbInvoices.create/update error", err);
    ...
    const handleAddLine = async () => {
        ...
        set("addingLine", true);
        try {
            await dbInvoices.addLine(invoice.id, {
            ...
            // Reload invoice so lines reflect the addition
            const refreshed = await dbInvoices.get(invoice.id);
            set("invoice", refreshed);
            set("lineLabel", "");
            set("lineQty", "1");
            set("lineSubprice", "0");
        } catch (err) {
            console.error("dbInvoices.addLine error", err);
    ...
    const handleDeleteLine = async (lineid) => {
        try {
            await dbInvoices.deleteLine(invoice.id, lineid);
            // Reload invoice so lines reflect the deletion
            const refreshed = await dbInvoices.get(invoice.id);
            set("invoice", refreshed);
        } catch (err) {
            console.error("dbInvoices.deleteLine error", err);
                    ...
                    <Block containerProps={{ className: "px-app-base mt-app-base md:px-6 md:max-w-4xl md:mx-auto" }} blockProps={{ className: "rounded-xl" }} title="En-tête">
                        <div className="flex flex-col gap-app-sm md:grid md:grid-cols-2 md:gap-4">
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="text-sm font-medium text-gray-600">Date facture</label>
                                <input
                                    type="date"
                                    value={datef ?? ""}
