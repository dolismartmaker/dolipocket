...
import { fmtAmount, fmtDate } from "./useSupplierInvoiceData";

// Default Dolibarr c_paiement entries -- same canonical list as the
// customer invoice page. Cf InvoicePage.desktop.jsx for rationale.
...
// Desktop rendering of the supplier invoice detail page. Same canonical
// layout as InvoicePage.desktop : sticky header (back/title/status/payment
// pill/actions) + 2-col body (lines left catalog-driven, side rail right
// with Informations catalog-driven + Totaux + Paiements + Notes hardcoded).
//
...
export const SupplierInvoicePageDesktop = (props) => {
    ...
    // The "Enregistrer paiement" button is only relevant for a supplier
    // invoice that is validated (statut >= 1) and not yet fully paid.
    const canPay = !!invoice && Number(invoice?.statut ?? 0) >= 1 && !isPaid;
    ...
    const defaultSubject = `Facture fournisseur ${refLabel}`.trim();
        ...
        <div className="flex flex-col h-full w-full bg-medium-bg overflow-hidden">
            <header className="shrink-0 flex items-center gap-3 px-4 py-2 border-b border-soft-border bg-white">
                ...
                <h1 className="text-base font-bold text-strong-text">
                    {loading ? "Chargement..." : (invoice?.ref || "Facture fournisseur")}
                </h1>
                {!loading && invoice && (
                    <>
                ...
                {!loading && invoice && (
                    <div className="flex items-center gap-2">
                        ...
                        {/* Send the supplier invoice by email with the last PDF attached. */}
                        <button
                            type="button"
                        ...
                        {/* Record a supplier payment against the invoice. */}
                        {canPay && (
                            <button
                                type="button"
                                onClick={openPayment}
                    ...
                    <div className="grid grid-cols-3 gap-4 max-w-[1400px] mx-auto">
                        ...
                        <aside className="col-span-1 flex flex-col gap-4">
                            <DocumentHeaderFields
                                ...
                                dataSource={dataSource}
                                storageKey="dolipocket.supplierinvoicepage.header"
                                title="Informations"
                                ...
                                <div className="px-4 py-2">
                                    ...
                                    {payments.length > 0 && (
                                        <div className="divide-y divide-soft-border/60">
                                            {payments.map((p, idx) => (
                                                <div key={p.id ?? idx} className="flex justify-between gap-3 py-1.5 text-[13px]">
