...
// Desktop edit page for a Supplier Invoice. Renders a single AutoForm
// generated from the backend describe() catalog. No JSX is hardcoded per
// field. Conventions UI épurées strictes (cf .claude/CLAUDE.md).
export const SupplierInvoiceEditPageDesktop = ({
    isNew,
    invoice,
    setInvoice,
    loading,
        ...
        <div className="flex flex-col h-full w-full bg-soft-bg overflow-hidden">
            ...
            <header className="sticky top-0 z-10 flex items-center gap-3 px-6 py-3 bg-white border-b border-soft-border">
                ...
                <h1 className="text-base font-semibold text-strong-text flex-1 truncate">
                    {isNew ? "Nouvelle facture fournisseur" : `Modifier ${invoice?.ref ?? ""}`}
                </h1>
                <button
                    type="button"
                    onClick={cancel}
                ...
                <div className="max-w-[1200px] mx-auto px-6 py-6">
                    ...
                    {loading ? (
                        <div className="rounded-md border border-soft-border bg-white p-4 text-sm text-soft-text">
                            Chargement de la facture fournisseur...
                        </div>
                    ) : (
                        ...
                        {/* Lines editor: only after the supplier invoice exists in DB.
                            On creation the user first saves the header (which
                            redirects to /supplier-invoices/:id/edit), then the editor
