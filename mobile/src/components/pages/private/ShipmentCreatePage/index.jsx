...
import { useShipmentCreateData } from "./useShipmentCreateData";

// "Create shipment from order" page (route /orders/:id/ship). Single responsive
// form: pick a warehouse + quantity per shippable order line, then POST it.
// Reached from the order detail page when the order is validated.
        ...
        <div className="flex flex-col h-full w-full bg-medium-bg overflow-hidden">
            <header className="shrink-0 flex items-center gap-3 px-4 py-2 border-b border-soft-border bg-white">
                ...
                <h1 className="text-base font-bold text-strong-text">
                    Nouvelle expédition{order?.ref ? ` - Commande ${order.ref}` : ""}
                </h1>
            ...
            <div className="flex-1 min-h-0 overflow-auto p-4">
                ...
                {!loading && !error && order && Number(order.statut) < 1 && (
                    <div className="mb-4 bg-amber-50 text-amber-800 border border-amber-200 rounded-md px-3 py-2 text-[13px]">
                        {"Cette commande n'est pas validée. Validez-la avant d'expédier pour un suivi de stock correct."}
                    </div>
                )}
                ...
                {!loading && !error && (lines?.length ?? 0) === 0 && (
                    <div className="bg-white rounded-xl border border-soft-border px-4 py-6 text-center text-soft-text text-[13px]">
                        Aucune ligne produit expédiable sur cette commande.
                    </div>
                )}
                    ...
                    <div className="flex flex-col gap-4 max-w-4xl">
                        <section className="bg-white rounded-xl border border-soft-border overflow-hidden">
                            <header className="px-4 py-2.5 border-b border-soft-border">
                                <h2 className="text-sm font-semibold text-strong-text">Lignes à expédier</h2>
                            </header>
                                ...
                                <table className="w-full text-[13px]">
                                    <thead>
                                        <tr className="text-left text-soft-text border-b border-soft-border">
                                            ...
                                            <th className="font-medium px-3 py-2 text-right w-28">Qté à expédier</th>
                                            <th className="font-medium px-3 py-2 w-56">Entrepôt</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {lines.map((l, idx) => (
                                            <tr key={l.fkOriginLine ?? idx} className="border-b border-soft-border/60">
                        ...
                        <section className="bg-white rounded-xl border border-soft-border overflow-hidden max-w-md">
                            <header className="px-4 py-2.5 border-b border-soft-border">
                                <h2 className="text-sm font-semibold text-strong-text">{"Détails de l'expédition"}</h2>
                            </header>
