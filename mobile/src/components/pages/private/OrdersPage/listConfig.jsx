...
export const ordersListConfig = {
    storageKey: "dolipocket.list.orders",
    rowKey: (row) => row.id,
    ...
    columnsOverrides: {
        ...
        socid:         { defaultVisible: true,  defaultWidth: 200 },
    ...
    rowActions: [
        {
            ...
            onClick: (row, ctx) => ctx.navigate(`/orders/${row.id}`),
        },
        {
            key: "edit", icon: FaPen, label: "Modifier", permission: "order.write",
            onClick: (row, ctx) => ctx.navigate(`/orders/${row.id}/edit`),
        },
    ],

    rowKebabActions: [
        {
            key: "delete", label: "Supprimer", danger: true, permission: "order.delete",
            confirm: ({ row }) => ({
                type: "delete",
            ...
            onClick: async (row, ctx) => {
                try {
                    await ctx.api.delete(`order/${row.id}`).json();
    ...
    bulkActions: [
        {
            key: "delete", label: "Supprimer", icon: FaTrash, danger: true, permission: "order.delete",
            confirm: ({ selected }) => ({
                type: "delete",
                title: `Supprimer ${selected.length} commande${selected.length > 1 ? "s" : ""} ?`,
                danger: true,
            }),
            run: async (rows, ctx) => {
                try {
                    const res = await ctx.api
                        .delete("order", { json: { ids: rows.map((r) => r.id) } })
                        .json();
                    ...
                    console.error("bulk delete orders", err);
                    ctx.toast.error("Suppression impossible");
                }
            },
        },
    ...
    headerActions: [
        {
            key: "new", label: "Nouvelle commande", icon: FaPlus, primary: true, permission: "order.create",
            onClick: (ctx) => ctx.navigate("/orders/new"),
        },
    ],
};
