...
// Data hook for the desktop SupplierOrderEditPage. The mobile variant is the
...
export const useSupplierOrderEditData = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        ...
        dbSO
            ...
            .catch((err) => {
                if (cancelled) return;
                console.error("[useSupplierOrderEditData] dbSO.get error", err);
    ...
    if (describeRef.current === null) {
        describeRef.current = ({ signal } = {}) => dbSO.describe({ signal });
    }
    ...
    const save = useCallback(async (values) => {
        ...
        try {
            if (isNew) {
                const data = await dbSO.create(values);
                if (data?.id) {
            ...
            const data = await dbSO.update(id, values);
            setOrder(data);
            return data;
        } catch (err) {
            console.error("[useSupplierOrderEditData] save error", err);
            setError("Erreur lors de l'enregistrement");
            return null;
        } finally {
            setSaving(false);
        }
    }, [dbSO, id, isNew, navigate]);
