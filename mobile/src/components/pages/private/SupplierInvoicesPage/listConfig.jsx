...
export const supplierInvoicesListConfig = {
    storageKey: "dolipocket.list.supplierInvoices",
    rowKey: (row) => row.id,
    ...
    rowActions: [
        {
            ...
            onClick: (row, ctx) => ctx.navigate(`/supplier-invoices/${row.id}`),
        },
        {
            key: "edit", icon: FaPen, label: "Modifier", permission: "supplierinvoice.write",
            onClick: (row, ctx) => ctx.navigate(`/supplier-invoices/${row.id}/edit`),
        },
    ],

    rowKebabActions: [
        {
            key: "delete", label: "Supprimer", danger: true, permission: "supplierinvoice.delete",
            confirm: ({ row }) => ({
                type: "delete",
            ...
            onClick: async (row, ctx) => {
                try {
                    await ctx.api.delete(`supplierinvoice/${row.id}`).json();
                    ctx.toast.success("Facture supprimée");
                    ctx.refresh();
                } catch (err) {
                    console.error("delete supplier invoice", err);
    ...
    bulkActions: [
        {
            key: "delete", label: "Supprimer", icon: FaTrash, danger: true, permission: "supplierinvoice.delete",
            confirm: ({ selected }) => ({
                type: "delete",
                title: `Supprimer ${selected.length} facture${selected.length > 1 ? "s" : ""} fournisseur ?`,
                danger: true,
            }),
            run: async (rows, ctx) => {
                try {
                    const res = await ctx.api
                        .delete("supplierinvoice", { json: { ids: rows.map((r) => r.id) } })
                        .json();
                    ...
                    console.error("bulk delete supplier invoices", err);
                    ctx.toast.error("Suppression impossible");
                }
            },
        },
    ...
    headerActions: [
        {
            key: "new", label: "Nouvelle facture fournisseur", icon: FaPlus, primary: true, permission: "supplierinvoice.create",
            onClick: (ctx) => ctx.navigate("/supplier-invoices/new"),
        },
    ],
};
